...
#include "CupSim/CupEllipsoid.hh"
...
CupEllipsoid::~CupEllipsoid() { ; }

// constructor - check parameters, convert angles so 0<sphi+dpshi<=2_PI
//             - note if pDPhi>2PI then reset to 2PI

CupEllipsoid::CupEllipsoid(const G4String &pName, G4double pRx, G4double pRy, G4double pRz,
                           G4double pZCut1, G4double pZCut2)
    : G4CSGSolid(pName) {
    ...
    if (pRx > 0. && pRy > 0. && pRz > 0.) {
        ...
        // G4Exception("Error in CupEllipsoid::CupEllipsoid - invalid radii");
        G4Exception(" ", " ", JustWarning, "Error in CupEllipsoid::CupEllipsoid - invalid radii");
    }

    if (pZCut1 < pRz && pZCut2 > -pRz && pZCut1 < pZCut2) {
        SetZCuts(pZCut1, pZCut2);
    } else {
        // G4Exception("Error in CupEllipsoid::CupEllipsoid - invalid z cut");
        G4Exception(" ", " ", JustWarning, "Error in CupEllipsoid::CupEllipsoid - invalid z cut");
    }
}
...
void CupEllipsoid::ComputeDimensions(G4VPVParameterisation *, const G4int,
                                     const G4VPhysicalVolume *) {
    G4cerr << "Warning: ComputeDimensions is not defined for CupEllipsoid. "
...
G4bool CupEllipsoid::CalculateExtent(const EAxis pAxis, const G4VoxelLimits &pVoxelLimit,
...
EInside CupEllipsoid::Inside(const G4ThreeVector &p) const {
    ...
    if (p.z() < fZCut1 - kRadTolerance / 2.0) return in = kOutside;
    if (p.z() > fZCut2 + kRadTolerance / 2.0) return in = kOutside;
    ...
    //
    if (rad2oi < 1.0) {
        in = (p.z() < fZCut1 + kRadTolerance / 2.0 || p.z() > fZCut2 - kRadTolerance / 2.0)
                 ? kSurface
                 : kInside;
    } else {
        in = kSurface;
...
G4ThreeVector CupEllipsoid::SurfaceNormal(const G4ThreeVector &p) const {
...
G4double CupEllipsoid::DistanceToIn(const G4ThreeVector &p, const G4ThreeVector &v) const {
    G4double distMin;
    ...
    B = 2.0 *
        (p.x() * v.x() / (fRx * fRx) + p.y() * v.y() / (fRy * fRy) + p.z() * v.z() / (fRz * fRz));

    C = B * B - 4.0 * A * C;
    if (C > 0.0) {
        G4double distR = (-B - sqrt(C)) / (2.0 * A);
        G4double intZ  = p.z() + distR * v.z();
        if (distR > kRadTolerance / 2.0 && intZ >= fZCut1 - kRadTolerance / 2.0 &&
...
G4double CupEllipsoid::DistanceToIn(const G4ThreeVector &p) const {
...
G4double CupEllipsoid::DistanceToOut(const G4ThreeVector &p, const G4ThreeVector &v,
            ...
            *validNorm = TRUE;
            switch (surface) {
                ...
                case kCurvedSurf: {
                    ...
                    G4ThreeVector truenorm(pexit.x() / (fRx * fRx), pexit.y() / (fRy * fRy),
                                           pexit.z() / (fRz * fRz));
                    truenorm *= 1.0 / truenorm.mag();
                    *n = truenorm;
                } break;
                default:
                    // G4Exception("Logic error in CupEllipsoid::DistanceToOut!");
                    G4Exception(" ", " ", JustWarning,
                                "Logic error in CupEllipsoid::DistanceToOut!");
                    break;
            }
        }
    }
...
G4double CupEllipsoid::DistanceToOut(const G4ThreeVector &p) const {
    G4double distR, distZ;
...
G4ThreeVectorList *CupEllipsoid::CreateRotatedVertices(const G4AffineTransform &pTransform,
                                                       G4int &noPolygonVertices) const {
    G4ThreeVectorList *vertices;
    G4ThreeVector vertex;
    G4double meshAnglePhi, meshRMaxFactor, crossAnglePhi, coscrossAnglePhi, sincrossAnglePhi,
        sAnglePhi;
    ...
    meshRMaxFactor = 1.0 / cos(0.5 * cup_hypot(meshAnglePhi, meshTheta));
    ...
    if (vertices && cosCrossTheta && sinCrossTheta) {
        ...
        for (crossSectionPhi = 0; crossSectionPhi < noPhiCrossSections; crossSectionPhi++) {
            ...
            for (crossSectionTheta = 0; crossSectionTheta < noThetaSections; crossSectionTheta++) {
                ...
                if (rz < fZCut1) rz = fZCut1;
                if (rz > fZCut2) rz = fZCut2;
        ...
        // G4Exception("CupSim/CupEllipsoid::CreateRotatedVertices Out of memory - Cannot alloc
        // vertices");
        G4Exception(
            " ", " ", JustWarning,
            "CupSim/CupEllipsoid::CreateRotatedVertices Out of memory - Cannot alloc vertices");
    }
...
void CupEllipsoid::DescribeYourselfTo(G4VGraphicsScene &scene) const {
#if (G4VERSION_NUMBER <= 470)
...
G4VisExtent CupEllipsoid::GetExtent() const {
    // Define the sides of the box into which the CupEllipsoid instance would fit.
    return G4VisExtent(-fRmax, fRmax, -fRmax, fRmax, -fRmax, fRmax);
}

// ================================================================

class CupPolyhedronEllipsoid : public G4Polyhedron {
  public:
    CupPolyhedronEllipsoid(G4double rx, G4double ry, G4double rz, G4double ZCut1, G4double ZCut2);
...
CupPolyhedronEllipsoid::CupPolyhedronEllipsoid(G4double rx, G4double ry, G4double rz,
 ...
 ***********************************************************************/
{
    ...
    if (ZCut1 >= rz || ZCut2 <= -rz || ZCut1 > ZCut2) {
        G4cerr << "CupSim/CupPolyhedronEllipsoid: wrong ZCut1 = " << ZCut1 << " ZCut2 = " << ZCut2
               << " for given rz = " << rz << G4endl;
        return;
    }
    if (rz <= 0.0) {
        G4cerr << "CupSim/CupPolyhedronEllipsoid: bad z radius: rz = " << rz << G4endl;
        return;
    }
    ...
    if (!zz || !rr) {
        // G4Exception("Out of memory in CupPolyhedronEllipsoid!");
        G4Exception(" ", " ", JustWarning, "Out of memory in CupPolyhedronEllipsoid!");
    }
    ...
    if (j > np1)
        // G4Exception("Logic error in CupPolyhedronEllipsoid, memory corrupted!");
        G4Exception(" ", " ", JustWarning,
                    "Logic error in CupPolyhedronEllipsoid, memory corrupted!");
    if (j < np1) {
        G4cerr << "Warning: logic error in CupPolyhedronEllipsoid." << G4endl;
        np1 = j;
    }
    zz[j] = 0.;
    rr[j] = 0.;
...
G4Polyhedron *CupEllipsoid::CreatePolyhedron() const {
    return new CupPolyhedronEllipsoid(fRx, fRy, fRz, fZCut1, fZCut2);
}
...
G4NURBS *CupEllipsoid::CreateNURBS() const {
    return new G4NURBSbox(fRmax, fRmax, fRmax); // Box for now!!!
}
#endif
// ******************************  End of CupEllipsoid.cc  ****************************************
